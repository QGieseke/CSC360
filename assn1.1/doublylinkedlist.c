...
void swap(struct node* ref1, struct node* ref2) {
	...
	struct node* ref1_prev = ref1->prev;
	struct node* ref2_prev = ref2->prev;
	struct node* ref1_next = ref1->next;
	struct node* ref2_next = ref2->next;
	...
	if(ref1_next != ref2 && ref2_next != ref1){
		//swap (safe since no derefrencing)
		ref1->prev = ref2_prev;
		ref1->next = ref2_next;
		
		ref2->prev = ref1_prev;
		ref2->next = ref1_next;
	//special case if adjacent 
	} else if (ref1_next == ref2){
		ref1->prev = ref2;
		ref1->next = ref2_next;
		
		ref2->prev = ref1_prev;
		ref2->next = ref1;
	//if they are adjacent, but 2 is first
	} else if (ref2_next == ref1){
		ref2->prev = ref1;
		ref2->next = ref1_next;
...
struct node* shift_left(struct node* ref, int n) {
	//assuming ref points to the head of the list
	/*struct node* temp = begin(ref);
	...
	*/
    ...
    do {
		i--;
        printf("%d: %p \t %p \t %p\n", temp->data, temp->prev, temp, temp->next);
    } while(((temp = next(temp)) != NULL) && i > 0);
    printf("\n");
	*/
    return begin(curr);
}
